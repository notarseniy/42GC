		...
		function getVisitors(count, callback) {
				link.query('SELECT SUM(visitors) FROM link',
					function (err, result) {
						...
						callback(null, {
							links: count,
							visitors: result[0]['SUM(visitors)']
						});
					});
		}
	], function (err, result) {
		...
		async.waterfall([
			function checkUrl(callback) {
					...
					link.findOne({
						...
						if (link) {
							callback({
								msg: 'Увы, но этот короткий адрес уже занят'
							});
						} else {
							callback(null);
						}
			...
			function createLink(callback) {
					shorten.delink = crypto.createHash('sha1')
						.update(shorten.sUrl + rand.generateKey(8))
						.digest('hex');
